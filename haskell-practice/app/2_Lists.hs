-- A list is a data structure that stores an ordered sequence of elements

-- Lists are denoted by square brackets ([ ]). For example, [1, 2, 3] is a list of integers 
-- Lists in Haskell are immutable, meaning once a list is created, it cannot be altered directly. 
-- However, we can create new lists by performing operations on existing ones.
...
-- This allows us to work with infinite lists, like [1..].
